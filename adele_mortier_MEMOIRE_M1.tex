...
%\usepackage[firstcite(document)]{footbib}
...
\begin{document}
	...
	\end{center}
		...
		\section*{Définition générale}
			...
			% a preciser que notre définitions est bcp plus large que la défintion de bootz, a completer par \guillemotleft~d'un ensemble de règles qui constituent une grammaire et d'un ensemble d'éléments préconstruits qui forment un dictionnaire.~\guillemotright~(Bootz, 2006) BOOTZ, Philippe (2006). «Vers de nouvelles formes en poésie numérique programmée?», RiLUnE ​ , ​ ​ n°5,​ ​ 2006.
			...
			Ensuite, cette littérature n'est pas directement maîtrisée dans la mesure où l'\textit{input} fourni à la machine (une \guillemotleft~graine~\guillemotright~de texte ou \textit{seed}, une série d'hyperparamètres\footnote{Par opposition aux simples paramètres (ou parfois \textit{coefficients}), les hyperparamètres sont, en apprentissage statistique, des paramètres entrés par le programmeur avant que l'entraînement du système ne commence. Ce sont donc des paramètres d'ordre supérieur comparés aux paramètres appris automatiquement par le système. Les hyperparamètres sont contrôlés par le programmeur et traduisent donc sa relative maîtrise du programme, même si dans certains cas la relation entre le choix des  hyperparamètres et la qualité des résultats finaux est difficile à appréhender. Un exemple \guillemotleft simple\guillemotright~ d'hyperparamètre est le taux de \textit{dropout} en sortie d'un réseau de neurones, c'est-à-dire la probabilité pour que certains neurones soient ignorés pendant la phase d'optimisation.}...), s'il existe, ne définit que très rarement ce qui va suivre à coup sûr ; et même si c'était le cas, la relation ne serait que très difficilement appréhendable par un humain. De plus, le processus de génération peut inclure des décisions randomisées (par exemple, dans quel nouvel état va passer une chaîne de Markov, quels sont les branchements précis d'un réseau de neurones...).\\
			\begin{figure}[H]
				\centering
				...
				\node [box, pin={[init]above:hyperparamètres}] (a) {\guillemotleft~programme~\guillemotright};
			...
			La définition de tels systèmes semble relativement complexe; et pourtant des processus satisfaisant ces conditions ne sont pas difficiles à construire, comme le montre le Tableau \ref{tab:dimensions}. La \guillemotleft~perte de contrôle~\guillemotright~qu'impose la littérature générative telle que nous l'avons définie peut ainsi être ménagée par des processus aléatoires simples~: un jet de dés, un livre ouvert au hasard.\\
			...
			\begin{table}[H]
				\begin{tabular}{c | c | c}
					...
					Contrôle direct & S+n, n tiré aux dés \footnotemark & Ecriture \guillemotleft~traditionnelle~\guillemotright\\
					Contrôle indirect & Chaîne de Markov & S+7, S pris au hasard dans le dictionnaire \footnotemark\\
				\end{tabular}
				\caption{Un tableau récapitulatif des \guillemotleft~dimensions~\guillemotright~de la littérature générative, avec des exemples}
				\label{tab:dimensions}
			\end{table}
			\footnotetext{Se rapproche de la contrainte \guillemotleft~S+dé~\guillemotright~de l'OuLiPo}
			\footnotetext{Se rapproche de la contrainte \guillemotleft~Eclipse~\guillemotright~de l'OuLiPo}
			
			%Il est difficile de comprendre si, dans cette partie, vous envisagez la littérature générative en elle-même, et donc sans prendre spécifiquement en compte ses développements informatiques, ou si vous intégrez dès le début cette dimension numérique.
	...
	%Contrainte de pascal => fond
				%https://gallica.bnf.fr/ark:/12148/bpt6k33246873/f63.image
...
%				OuLiPo, un mouvement anti-hasard... Au contraire de max bense. À propos de son roman La Vie mode d'emploi (1978), Georges Perec parlait cependant d'\guillemotleft~aléatoire mécanique~\guillemotright~et de \guillemotleft~programmation du hasard~\guillemotright~: d'une littérature stochastique, en quelque sorte. \guillemotleft~Il n'y a de littérature que volontaire~\guillemotright, écrivait Raymond Queneau [Oulipo, 1973~: 27])
...
%				\begin{citationbox}
%					\textit{S'intéresser aux rapports entre littérature et informatique, ce sera dès lors rendre compte des implications complexes entre texte, machine et programme. Comme nous pouvons déjà le suspecter, il ne sera plus possible de parler de texte de façon autonome~: il n'y aura plus de texte. Il y aura en revanche un système \guillemotleft~texte-programme-machine~\guillemotright, une configuration complexe dont les frontières resteront floues et intriquées.}
%					\begin{flushright}
%						Ambroise Barras, \textit{Prolégomènes à toute littérature informatique} \autocite{barras1995}
%					\end{flushright}
%				\end{citationbox}
...
%hypertexte Jardin aux sentiers qui bifurquent borges bonnet
	...
	\section{Aspects techniques de la littérature générative}\label{aspects_tech}
		...
		\subsection{Les méthodes \textit{rule-based}}
			...
			\subsubsection{Machines de Turing, automates et transducteurs}\label{automates}
				...
				\end{figure}
				\footnotetext{Merci au contributeur GabrielF}
				...
				Les automates sont un exemple de théorie plus riche et Turing-complète. Ici le terme peut paraître trompeur, nous ne parlerons pas des automates \guillemotleft~robots~\guillemotright~comme ceux de  Jaquet-Droz~! Pour simplifier, un automate fini est un genre de graphe orienté, composé~:
				\vspace{2mm}
				\begin{itemize}
					\item d'un \textit{input}, qui est une chaîne de symboles. Les symboles sont lus un par un et \guillemotleft~défaussés~\guillemotright~après lecture;
				...
				\begin{figure}[H]
					...
					\end{tikzpicture}
					\caption{Un automate très simple reconnaissant les verbes substantivés comme \guillemotleft~activation~\guillemotright, \guillemotleft~réactivation~\guillemotright, \guillemotleft~réréactivation~\guillemotright, \guillemotleft~désactivation~\guillemotright~etc. (PREFIX = ré$|$dé$|$anti$|$sur... VS = activa$|$consomma$|$constitu... SUFFIX = tion)}
					\label{fig:automata}
				\end{figure}
				Pour aborder la génération, on utilise un outil un peu plus puissant que l'automate, le transducteur. Un transducteur est, de façon très simplifiée, un automate qui en plus de vérifier chaque symbole \guillemotleft~écrit~\guillemotright~un symbole en retour. Les transitions d'un transducteur sont donc étiquetées non plus par un symbole unique, mais par un couple formé du symbole à reconnaître (typiquement, une classe grammaticale, une catégorie de morphème...) et du symbole à retourner (typiquement, un léxème ou un morphème).\\
				\begin{figure}[H]
					\centering
				...
				\end{figure}
				On voit ainsi que les transducteurs peuvent être le concept sous-jacent à l'idée des \guillemotleft~phrases à trous~\guillemotright~utilisées par Strachey, Lutz et L'OuLiPo. Toutefois les transducteurs gèrent difficilement les conflits liés à la combinatoire. Par exemple, le tranducteur simpliste présenté Figure \ref{fig:transducer} génère des mots corrects comme \guillemotleft~surconsommation~\guillemotright, \guillemotleft~réactivation~\guillemotright, \guillemotleft~constitution~\guillemotright, mais génère aussi une myriade de formes incorrectes comme \guillemotleft~déantiactivation~\guillemotright. La gestion des cas particuliers et des incompatibilités conduit très rapidement à une multiplication des états possibles et des transitions -- ce qui dépasse souvent la capacité humaine. C'est pourquoi des outils comme les transducteurs peuvent difficilement se passer d'une correction manuelle \textit{a posteriori} par l'auteur.
		...
		\subsection{Les méthodes statistiques}\label{methodes_stat}
			...
			\subsubsection{Le modèle du \guillemotleft~sac de mots continu~\guillemotright}\label{cbow}
				...
				table[meta=Mot]
				{
					...
					.9 .9 \guillemotleft~Grandiose~\guillemotright
					.2 .2 \guillemotleft~Nul~\guillemotright
					.1 .2 \guillemotleft~Moche~\guillemotright
					.7 .8 \guillemotleft~Superbe~\guillemotright
					.7 .85 \guillemotleft~Magnifique~\guillemotright
					.7 .3 \guillemotleft~Laid~\guillemotright
					.3 .5 \guillemotleft~Beau~\guillemotright
					.7 .1 \guillemotleft~Ignoble~\guillemotright
					.1 .1 \guillemotleft~Dégueu~\guillemotright
					.1 .6 \guillemotleft~Stylé~\guillemotright
				};
				\end{axis}
				\end{tikzpicture}
			...
			\subsubsection{Les réseaux de neurones}\label{neural_net}
				\subsubsubsection{Présentation générale}
					...
					\begin{figure}[H]
						...
						\end{tikzpicture}
						\caption{Un réseau de neurones simple a $3$ couches. Chaque cercle est un neurone, les arc entre les cercle sont des \guillemotleft~synapses~\guillemotright}
	...
	\section*{Remerciements}
		Je remercie Monsieur Michel Bernard, professeur à l'Université Paris 3, pour le temps qu'il m'a consacré et l'aide qu'il a bien voulu m'apporter dans la rédaction de ce mémoire. Je le remercie en particulier pour sa relecture attentive de mon travail, ses commentaires enrichissants et ses conseils précieux.
	\newpage
	...
	\defbibheading{corpusmain}{\textbf{Supports principaux}}
	\defbibheading{corpusminor}{\textbf{Supports secondaires}}
	\defbibheading{histlit}{\textbf{Histoire littéraire}}
	\defbibheading{poephilo}{\textbf{Poétique et philosophie}}
	\defbibheading{scitech}{\textbf{Science et technique}}
	\defbibheading{sites}{\textbf{Sitographie}}
	\defbibheading{misc}{\textbf{Autres supports}}
	\section*{Bibliographie}
		\subsection*{Corpus}
			\printbibliography[heading=corpusmain, keyword=corpus]
			\printbibliography[keyword=minor, heading=corpusminor]
		 \subsection*{Articles}
			\printbibliography[keyword=article, heading=histlit]
			\printbibliography[keyword=theo, heading=poephilo]
			\printbibliography[keyword=tech, heading=scitech]
		\subsection*{Divers}
			\printbibliography[keyword=site, heading=sites]
			\printbibliography[keyword=misc, heading=misc]
	...
	\section{Annexes}
		\subsection{Pseudo code de l'algorithme \textit{Love Letters} de Christopher Strachey}\label{strachey_code}
			\begin{algorithm}
				...
				\begin{algorithmic}[1]
					...
					\State {sals$_1$ $\gets$ [\guillemotleft~Beloved~\guillemotright, \guillemotleft~Darling~\guillemotright, \guillemotleft~Dear~\guillemotright, \guillemotleft~Dearest~\guillemotright, \guillemotleft~Fanciful~\guillemotright, \guillemotleft~Honey~\guillemotright]}
					\State
					\State {sals$_2$ $\gets$ [\guillemotleft~Chickpea~\guillemotright, \guillemotleft~Dear~\guillemotright, \guillemotleft~Duck~\guillemotright, \guillemotleft~Jewel~\guillemotright, \guillemotleft~Love~\guillemotright, \guillemotleft~Moppet~\guillemotright, \guillemotleft~Sweetheart~\guillemotright]}
					\State
					\State {adjs $ \gets $ [\guillemotleft~affectionate~\guillemotright, \guillemotleft~amorous~\guillemotright, \guillemotleft~anxious~\guillemotright, \guillemotleft~avid~\guillemotright, \guillemotleft~beautiful~\guillemotright, \guillemotleft~breathless~\guillemotright, \guillemotleft~burning~\guillemotright, \guillemotleft~covetous~\guillemotright, \guillemotleft~craving~\guillemotright, \guillemotleft~curious~\guillemotright, \guillemotleft~eager~\guillemotright, \guillemotleft~fervent~\guillemotright, \guillemotleft~fondest~\guillemotright, \guillemotleft~loveable~\guillemotright, \guillemotleft~lovesick~\guillemotright, \guillemotleft~loving~\guillemotright, \guillemotleft~passionate~\guillemotright, \guillemotleft~precious~\guillemotright, \guillemotleft~seductive~\guillemotright, \guillemotleft~sweet~\guillemotright, \guillemotleft~sympathetic~\guillemotright, \guillemotleft~tender~\guillemotright, \guillemotleft~unsatisfied~\guillemotright, \guillemotleft~winning~\guillemotright, \guillemotleft~wistful~\guillemotright]}
					\State
					\State {nouns $\gets$ [\guillemotleft~adoration~\guillemotright, \guillemotleft~affection~\guillemotright, \guillemotleft~ambition~\guillemotright, \guillemotleft~appetite~\guillemotright, \guillemotleft~ardour~\guillemotright, \guillemotleft~being~\guillemotright, \guillemotleft~burning~\guillemotright, \guillemotleft~charm~\guillemotright, \guillemotleft~craving~\guillemotright, \guillemotleft~desire~\guillemotright, \guillemotleft~devotion~\guillemotright, \guillemotleft~eagerness~\guillemotright, \guillemotleft~enchantment~\guillemotright, \guillemotleft~enthusiasm~\guillemotright, \guillemotleft~fancy~\guillemotright, \guillemotleft~fellow feeling~\guillemotright, \guillemotleft~fervour~\guillemotright, \guillemotleft~fondness~\guillemotright, \guillemotleft~heart~\guillemotright, \guillemotleft~hunger~\guillemotright, \guillemotleft~infatuation~\guillemotright, \guillemotleft~little liking~\guillemotright, \guillemotleft~longing~\guillemotright, \guillemotleft~love~\guillemotright, \guillemotleft~lust~\guillemotright, \guillemotleft~passion~\guillemotright, \guillemotleft~rapture~\guillemotright, \guillemotleft~sympathy~\guillemotright, \guillemotleft~thirst~\guillemotright, \guillemotleft~wish~\guillemotright, \guillemotleft~yearning~\guillemotright]}
					\State
					\State {advs $\gets$ [\guillemotleft~affectionately~\guillemotright, \guillemotleft~ardently~\guillemotright, \guillemotleft~anxiously~\guillemotright, \guillemotleft~beautifully~\guillemotright, \guillemotleft~burningly~\guillemotright, \guillemotleft~covetously~\guillemotright, \guillemotleft~curiously~\guillemotright, \guillemotleft~eagerly~\guillemotright, \guillemotleft~fervently~\guillemotright, \guillemotleft~fondly~\guillemotright, \guillemotleft~impatiently~\guillemotright, \guillemotleft~keenly~\guillemotright, \guillemotleft~lovingly~\guillemotright, \guillemotleft~passionately~\guillemotright, \guillemotleft~seductively~\guillemotright, \guillemotleft~tenderly~\guillemotright, \guillemotleft~wistfully~\guillemotright]}
					\State
					\State {verbs $\gets$ [\guillemotleft~adores~\guillemotright, \guillemotleft~attracts~\guillemotright, \guillemotleft~clings to~\guillemotright, \guillemotleft~holds dear~\guillemotright, \guillemotleft~hopes for~\guillemotright, \guillemotleft~hungers for~\guillemotright, \guillemotleft~likes~\guillemotright, \guillemotleft~longs for~\guillemotright, \guillemotleft~loves~\guillemotright, \guillemotleft~lusts after~\guillemotright, \guillemotleft~pants for~\guillemotright, \guillemotleft~pines for~\guillemotright, \guillemotleft~sighs for~\guillemotright, \guillemotleft~tempts~\guillemotright, \guillemotleft~thirsts for~\guillemotright, \guillemotleft~treasures~\guillemotright, \guillemotleft~yearns for~\guillemotright, \guillemotleft~woos~\guillemotright]}
					\State
					\State {salutation $ \gets $ random\_element(sals$_1$) + \guillemotleft~\~\guillemotright~+ random\_element(sals$_2$) }
					\State
					\For{i $\in$ [1, 5]}
						{\State {long $ \gets $ bernoulli(.5)}
						\If{long}
							\State {phrase $ \gets $ \guillemotleft~My~\guillemotright~+ random\_element(adjs) + \guillemotleft~\~\guillemotright~+ random\_element(nouns) + \guillemotleft~\~\guillemotright~+  random\_element(advs) + \guillemotleft~\~\guillemotright  + random\_element(verbs) + \guillemotleft~your~\guillemotright~+ random\_element(adjs) + \guillemotleft~\~\guillemotright~+ random\_element(nouns)}
						\Else
							\State{phrase $ \gets $ \guillemotleft~You are my~\guillemotright~+ random\_element(adj) + \guillemotleft~\~\guillemotright~+ random\_element(noun)}
						\EndIf}
						\State {salutation $ \gets $ salutation + phrase}
					\EndFor
	
					\State {closure $ \gets $ \guillemotleft~Yours~\guillemotright~+ random\_element(adv) + \guillemotleft~, M.U.C.~\guillemotright}
					\State
					\Return {salutation + \guillemotleft~\textbackslash n~\guillemotright~+ closure}
					\EndProcedure
				\end{algorithmic}
			\end{algorithm}
			\newpage
		...
		\subsection{Textes \guillemotleft~à la manière de Hugo~\guillemotright~générés par nos soins avec un réseau LSTM}\label{lstm_hugo_ex}
			\textbf{Graine} : [Le chiffre de mes ans a passé q]\guillemotleft~\textbf{uatre vingt, et je n'ai pas de fils, e}~\guillemotright[t je n'ai plus de femme.] (source : \textit{Booz endormi})
